% function best_fitness = BSO(fobj,N,dim,n_c,lb,ub,Max_FEs)
function best_fitness = BSO(N, Max_FEs, lb, ub, dim, fobj)
...
stepSize = ones(1,dim); % effecting the step size of generating new individuals by adding random values
popu = lb + (ub - lb) * rand(N,dim); % initialize the population of individuals
popu_sorted  = lb + (ub - lb) * rand(N,dim); % initialize the  population of individuals sorted according to clusters
...
centers = lb + (ub - lb) * rand(n_c,dim);  % initialize best individual in each cluster
centers_copy = lb + (ub - lb) * rand(n_c,dim);  % initialize best individual-COPY in each cluster FOR the purpose of introduce random best
best_fitness = 1000000*ones(Max_FEs,1);
fitness_popu = 1000000*ones(N,1);  % store fitness value for each individual
fitness_popu_sorted = 1000000*ones(N,1);  % store  fitness value for each sorted individual
indi_temp = zeros(1,dim);  % store temperary individual
% calculate fitness for each individual in the initialized population
for idx = 1:N
    fitness_popu(idx,1) = fobj(popu(idx,:));
end
while n_iteration < Max_FEs
    ...
    if (rand() < 0.2) %  select one cluster center to be replaced by a randomly generated center
        ...
        centers(cenIdx,:) = lb + (ub - lb) * rand(1,dim);
    end 
           
    % calculate cluster probabilities based on number of individuals in
    % each cluster
    for idx = 1:n_c
        prob(idx,1) = number_in_cluster(idx,1)/N;
        if idx > 1
    ...
    for idx = 1:N
        ...
        stepSize = logsig(((0.5*Max_FEs - n_iteration)/20)) * rand(1,dim);
        indi_temp(1,:) = indi_temp(1,:) + stepSize .* normrnd(0,1,1,dim);
