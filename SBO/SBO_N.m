...
%             r3 = tanh(Max_FEs/(Max_FEs - randn * FEs));
%             r3 = tanh((sqrt(abs(Max_FEs - randn * FEs))/i)^(FEs/Max_FEs));
%             r3 = tanh((randn * FEs/Max_FEs)^2);
%             r3 = real(r3);
%             r3 = tanh(FEs/Max_FEs * (Max_FEs - FEs)/(FEs/N));
%             r3 = tanh(((Max_FEs - FEs)/(FEs/N))^(FEs/Max_FEs));
%             r3 = tanh((abs(Max_FEs - randn * FEs)/i)^(FEs/Max_FEs));
%             r3 = tanh(((abs(Max_FEs - randn * FEs)/(i * N)))^(FEs/Max_FEs));
%             r3 = tanh(((Max_FEs - FEs)/i)^(FEs/Max_FEs));
%             r3 = tanh(((abs(Max_FEs - randn * FEs)/(i * N)))^(FEs/Max_FEs));
%             r3 = tanh((sqrt(abs(rand * Max_FEs - randn * FEs))/N)^(rand * FEs/Max_FEs));
        
%             r4 = tanh((Max_FEs - FEs)/(N * FEs));

%             r3 = tanh(((abs(Max_FEs - randn * FEs)/(i * N)))^(FEs/Max_FEs));
%             r3 = tanh(abs(Max_FEs - randn * FEs)/N);
%             r3 = tanh(((Max_FEs - FEs)/N)^(FEs/Max_FEs));


            tmp = ['abs r3 = ', num2str(r3)];
            disp(tmp);
            r4 = unifrnd((1-FEs/Max_FEs)^2, (1-FEs/Max_FEs)^2);
%             r5 = unifrnd(-r4, r4);
            tmp2 = ['abs r4 = ', num2str(r4)];
%             disp(tmp2);
            if rand < r3
                for j = 1:dim
...
%                     current_X(i, j) = r5 * ((1 - r1 - r2) * current_X(i, j) + r1 * localElite_X(Rolette_index, j) + r2 * best_pos(j));
                end
            end
        end
    ...
    disp(record);
    tmp3 = ['SBO_N the operation2 execution count = ', num2str(count)];
    disp(tmp3);
end
