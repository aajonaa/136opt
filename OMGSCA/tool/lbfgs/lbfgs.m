function [x,fx,msg] = lbfgs(f,x0,options)

% LBGFS - Limited memory quasi-Newton method for solving
...
%          2. x0 - initial guess
%          3. options - struct containing solver options. For now, see code
%                       below for available options. Type "type lbfgs"
%                       or "edit lbfgs".
%
...
if nargin<2
    error('lbfgs requires at least two input arguments');
end
...
defaults = ...
    ...
    'max_linesearch',30,... % The maximum number of trials for the line search.
    ...
    'Display','iter');          % Options for displaying progress
...
if nargin>2
    if isstruct(options)
        ...
        error('Third argument must be a struct');
    end
end
...
if ~strcmp(defaults.Display,'none')
    ...
    switch msg
        ...
        case -997
            disp(['The maximum number of iterations (' num2str(defaults.MaxIter) ') was reached.']);
        case -996
