...
% 
function [GBEST,cg_curve]=GBMGSCA_V_MaxFEs(N,MaxFEs,lb,ub,dim,fobj)
if (size(ub, 2)~=1)
    dim = size(ub, 2);
else
    lb = lb*ones(1,dim);
        ...
        for j = 1 : Ssub
            ...
            for k = 1 : dim
                if rand() < CR
                    ...
                    V2(j, k) = normrnd(mu,sigma);
                else
                    V2(j, k) = gBest(i, k)+K*(Swarm(i, k1, k)-Swarm(i, k2, k));%%DE/best/1
                end
            end
            Vfitness(i, j) = fobj(V2(j, :));
            fes = fes + 1;
            if Vfitness(i, j) < Objective_values(i, j)
                Swarm(i, j, :) = V2(j, :);
            end
