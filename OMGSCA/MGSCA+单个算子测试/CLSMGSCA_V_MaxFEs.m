...
% 
function [GBEST,cg_curve]=CLSMGSCA_V_MaxFEs(N,MaxFEs,lb,ub,dim,fobj)
if (size(ub, 2)~=1)
    dim = size(ub, 2);
else
    lb = lb*ones(1,dim);
...
%             setCan = (iter-t+1)/iter;
            ...
            for ant=1:(K)
                ...
                V2 = (1-setCan)*gBest(i, :)+setCan*CH(ant);
               % ObjValV=feval(objfun,V);         %计算函数值
                Flag4ub=V2>ub;
                Flag4lb=V2<lb;
                V2=(V2.*(~(Flag4ub+Flag4lb)))+ub.*Flag4ub+lb.*Flag4lb;
                FitnessV=fobj(V2);%计算适应度值
                fes = fes + 1;
