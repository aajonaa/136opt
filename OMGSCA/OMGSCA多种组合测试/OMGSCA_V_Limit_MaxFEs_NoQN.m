...
% 
function [GBEST,cg_curve]=OMGSCA_V_Limit_MaxFEs_NoQN(N,MaxFEs,lb,ub,dim,fobj)
if (size(ub, 2)~=1)
    dim = size(ub, 2);
else
    lb = lb*ones(1,dim);
    ...
    for i = 1 : Nsub    
        ...
        for j = 1 : Ssub
            for k = 1 : dim
                ...
                if r4<0.5
                    % Eq. (3.1)
                    V(i,j,k)= Swarm(i,j,k)+(r1*sin(r2)*abs(r3*gBest(i,k)-Swarm(i,j,k)));
                else
                    % Eq. (3.2)
                    V(i,j,k)= Swarm(i,j,k)+(r1*cos(r2)*abs(r3*gBest(i,k)-Swarm(i,j,k)));
                end
        ...
        for j = 1 : Ssub
            if Trial(i, j) >= Limits
                ...
                [ BestPosition,BestFitness ,fes ] = Con_OEDCanSolution_For_OMGSCA( x, gBest(i, :), dim, Q, F, fobj , fes );
                if BestFitness < Objective_values(i, j)
                    Swarm(i, j, :) = BestPosition;
                    Objective_values(i, j) = BestFitness;
                else
                    for k = 1 : dim
                        Swarm(i, j, k) = LB(k)+rand()*(UB(k)-LB(k));
                    end
                end
                Trial(i, j) = 0;
            end
