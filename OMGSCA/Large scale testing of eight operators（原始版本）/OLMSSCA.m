...
% 
function [cg_curve]=OLMSSCA(N,Max_iteration,lb,ub,dim,fobj,funcNum)
...
while ( t < iter )
    ...
    for i = 1 : Nsub    
        ...
        for j = 1 : Ssub
            ...
            [ k1,k2,k3 ] = GetRan3( i,Ssub );
            T2 = zeros(1, dim);
            for k=1:dim
                T2(k)=Swarm(i,k1,k)+rand()*(Swarm(i,k2,k)-Swarm(i,k3,k));
            end
            ...
            [ BestPosition,BestFitness ] = Con_OEDCanSolution( x,T2,dim,Q,F,fobj );
            if BestFitness < fobj(x)
