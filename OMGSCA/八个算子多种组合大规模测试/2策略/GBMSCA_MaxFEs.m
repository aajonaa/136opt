...
% 
function [GBEST,cg_curve]=GBMSCA_MaxFEs(N,MaxFEs,lb,ub,dim,fobj)
if (size(ub, 2)~=1)
    dim = size(ub, 2);
else
    lb = lb*ones(1,dim);
...
while ( fes < MaxFEs )
    ...
    for i = 1 : Nsub    
        ...
        for j = 1 : Ssub
            ...
            Objective_values(i, j) = fobj(x);
            %           CR = 1-exp(-abs(Objective_values(1,i)-Destination_fitness));%%变异控制变量
            CR = 0.3;
            [ k1,k2 ] = GetRan2( j, Ssub );
            K = rand();
            for k = 1 : dim
                if rand() < CR
                    mu = (gBest(i, k)+Swarm(i, j, k)) / 2;
                    sigma = abs(gBest(i, k)-Swarm(i, j, k));
                    V(j, k) = normrnd(mu,sigma);
                else
                    V(j, k) = gBest(i, k)+K*(Swarm(i, k1, k)-Swarm(i, k2, k));%%DE/best/1
                end
            end
            Vfitness(i, j) = fobj(V(j, :));
            fes = fes + 1;
    ...
    % DNS
    [fes, Nsub, Ssub, m, Swarm, vel, pBestScore, pBest, gBestScore, gBest, GBESTSCORE, GBEST, gBestIndex, Trial, Objective_values] = DNS(fes, t, Cgen, N, Nsub, Ssub, m, NsubVector, Swarm, vel, pBestScore, pBest, gBestScore, gBest, GBESTSCORE, GBEST, fobj, dim, gBestIndex, Trial, Objective_values);
    VFitness=zeros(Nsub, Ssub);
    
    % SRS
    [Stag, Nsub, Ssub, Swarm, vel, pBestScore, pBest, gBestScore, gBest, Trial, Objective_values] = SRS(Stag, Nsub, Ssub, Swarm, vel, pBestScore, pBest, gBestScore, gBest, N, dim, Trial, Objective_values);
...
function [fes, Nsub, Ssub, m, Swarm, vel, pBestScore, pBest, gBestScore, gBest, GBESTSCORE, GBEST, gBestIndex, Trial, Objective_values] = DNS(fes, t, Cgen, N, Nsub, Ssub, m, NsubVector, Swarm, vel, pBestScore, pBest, gBestScore, gBest, GBESTSCORE, GBEST, fobj, dim, gBestIndex, Trial, Objective_values)
...
function [Stag, Nsub, Ssub, Swarm, vel, pBestScore, pBest, gBestScore, gBest, Trial, Objective_values] = SRS(Stag, Nsub, Ssub, Swarm, vel, pBestScore, pBest, gBestScore, gBest, N, dim, Trial, Objective_values)
if (Stag >= Ssub/2)
    SubSwarmIndex = randperm(N);
    SwarmL = Swarm;
    Swarm = zeros(Nsub, Ssub, dim);
