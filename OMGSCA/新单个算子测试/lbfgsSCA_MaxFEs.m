...
while ( fes < MaxFEs )
    ...
    for i = 1 : Nsub    
        ...
        defaults = ...
            ...
            'MaxIter',MaxFunEvals,... %The maximum number of iterations.
            'linesearch','more_thuente',...  % The line search algorithm.
            'max_linesearch',MaxFunEvals,... % The maximum number of trials for the line search.
        ...
        [x, fx, ~] = lbfgs(fobj, gBest(i, :)',defaults);
        x = x';
        if ( fx < gBestScore(i) )
            gBestScore(i) = fx;
            gBest(i, :) = x;
