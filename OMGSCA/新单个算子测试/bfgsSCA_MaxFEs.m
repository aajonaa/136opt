...
while ( fes < MaxFEs )
    ...
    for i = 1 : Nsub    
        ...
        % bfgs
        if ( (gBest(i, :) ~= lb) .* (gBest(i, :) ~= ub) )
            'bfgs'
            options=optimset('TolFun',1e-15,'TolX', 1e-15, 'HessUpdate','bfgs','MaxFunEvals', floor(0.1*fes));
            [outX, outf, ~, output] = fminunc(fobj,gBest(i, :),options);
