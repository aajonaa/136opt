...
%             else
%                 for j = 1:dim
%                     current_X(i, j) = (1 - r1 - r2) * current_X(i, j) - r1 * localElite_X(i, j) + r2 * best_pos(j);
%                 end
%                 m = zeros(1, dim);
%                 m(1, randi(dim)) = 1;
%                 for j = 1:dim
%                     if m(j) == 1
%                         current_X(i, j) = randn * localElite_X(Rolette_index, j); %%有效
%                     end
%                 end
                    % Sample from a normal distribution with mean at the antlion's position
                    % and a dynamically increasing standard deviation
%                     current_X(i, j) = normrnd(current_X(i, j), std_dev, 1, 1);

                    % Enforce upper and lower bounds
%                     current_X(i, j) = max(lb(j), min(ub(j), current_X(i, j)));
%                 end
%                 for j = 1:dim
% %                     current_X(i, j) = (1 - r1 - r2) * current_X(i, j) + r1 * localElite_X(i, j) + r2 * best_pos(j);
% %                     current_X(i, j) = (1 - r1 - r2) * best_pos(j) + r1 * localElite_X(i, j) + r2 * current_X(i, j);
%                     current_X(i, j) = (1-FEs/Max_FEs) * (1 - r1 - r2) * best_pos(j);
%                 end
%             end
        end
