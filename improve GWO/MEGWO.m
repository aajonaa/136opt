...
function [Alpha_pos,Cbest]=MEGWO(SearchAgents_no,Max_iter,lb,ub,dim,f)
...
while Fes<M
    pa=1-(1-0.6)*Fes/M;
    freq=0.4-(0.4-0)*Fes/M;
    a=2*(1-Fes/M); % a decreases linearly fron 2 to 0
    ...
    Positions=ControlBoundD(Positions,lb,ub);%防止越界
    %Positions=ControlBoundD(Positions,aa,bb);%防止越界
	Fes = Fes + 1;
   % fit=feval(f, Positions);%计算适应度值
    for i=1:size(Positions,1)
       fit(i) = feval(f,Positions(i,:));
    end
    for i=1:N        
        fitness=fit(i);
        if X(i,d+1)>fitness
