...
% 前期在GWO、HHO和Levy三者之间贪心选择最优个体，后期用CMA
% Grey Wolf Optimizer
function [Alpha_pos,Convergence_curve]=GWOCMAHHOL(SearchAgents_no,MaxFEs,lb,ub,dim,fobj)
    ...
    if FEs < MaxFEs/3               
		...
		for i=1:size(Positions,1)
		   ...
		   if abs(E) >= 1
			   ...
			   if q < 0.5
				   %display(q);
				   Positions1(i,:) = Pos_temp - rand()*abs(Pos_temp - 2 * rand() * Positions(i,:));
			   elseif q >= 0.5
				   %display(q);
				   Positions1(i,:) = (Alpha_pos - mean(Positions) - rand()*(ub - lb)*rand() + lb);
			   end
		   elseif abs(E) < 1
			   r = rand();
			   if r >= 0.5 && abs(E) < 0.5
				  % display(r);
				   Positions1(i,:) = Alpha_pos - E * abs(Alpha_pos - Positions(i,:));
			   end
			   if r >= 0.5 && abs(E) >= 0.5
				   J = 2 * (1 - rand());
				   %display(r);
				   Positions1(i,:) = (Alpha_pos -  Positions(i,:)) - E * abs(J * Alpha_pos - Positions(i,:));
			   end
			   if r < 0.5 && abs(E) >= 0.5
				   J = 2 * (1 - rand());
...
%                 temp(1,j)=(X1+X2+X3)/3;% Equation (3.7) 
        ...
        [ositions,FEs]=Best_Position(Positions,Positions1,Positions2,fobj,FEs); 
    else
        X = Positions;
        m = dim;
        n = SearchAgents_no;
