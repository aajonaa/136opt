...
%开始用混沌初始化，前期在Beta变异，Levy和GWO更新中选择最有个体，后期用CMA
% Grey Wolf Optimizer
function [Alpha_pos,Convergence_curve]=GWOCMALevyBeta(SearchAgents_no,MaxFEs,lb,ub,dim,fobj)
...
ch(1) = x;
for ant=1:(SearchAgents_no)
 	%ch(ant+1)=4*ch(ant)*(1-ch(ant)); %原始Logistic
    ch(ant+1)=5*(1+1/4)^4*ch(ant)*(1-ch(ant))^4;    %改进Logistic映射 式（4-1），ch即为该式中的βi
	%ch(ant + 1) = cos(1/ch(ant)^2);    %Fuch 映射
	%ch(ant + 1) = cos(4*acos(ch(ant)));%chebyshev映射
	%% cat 映射
% 	 X0 = [0.1,0.4];
% 	 x = zeros(1,SearchAgents_no);
% 	 y = x;
% 	 x(1) = X0(1);
% 	 y(1) = X0(2);
...
% 	 end
% 	 [C,ch]=hist(x,SearchAgents_no);
% 	%%

    PCh = ch(ant)*Positions;         %式（4-2）
    PHe = [Positions;PCh];%phe为混沌变化后的搜索代理位置和变化前的的位置
...
%     FitnessHe1=calculateFitness(ObjValHe1);
    [~,index] = sort(FitnessHe1);%FitnessHe2保存的是排序后的适应度值，index是FitnessHe2中保存的FitnessHe1的索引
    Positions = PHe(index,:);
...
while  FEs < MaxFEs
    ...
    if FEs < MaxFEs/2               
        ...
        for i=1:size(Positions,1)
            ...
            %display('size');
            %display(size(Positions,1));
            %display('Position');
            %display(Positions(i,:));
            %display('beta');
            %display((1+betarnd(1.2,1.2)));
            %display('X1');
            %tempX1 = Positions(i,:) *(1+betarnd(1.2,1.2));
            %display(tempX1);
            %X1 = [X1;tempX1];
            Positions1(i,:) = Positions(i,:) *(1+betarnd(1.2,1.2));
            %X1 = [X1;tempX1];
        end
%         for i=1:size(Positions,1)
%             
%         end
        Positions2=Lenvy(Positions,1);
        [Positions,FEs]=Best_Position(Positions, Positions1,Positions2,fobj,FEs);
...
function [X,FEs]=Best_Position(X,X1,X2,fobj,FEs)
n=size(X,1);
for i=1:n
	FEs = FEs + 3;
    y_X=fobj(X(i,:)); 
