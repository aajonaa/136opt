...
%　将GWO HHO 和CMA按照学长的方式在三者之间选择最优
% Grey Wolf Optimizer
function [Alpha_pos,Convergence_curve]=GWOCMAHHOorder(SearchAgents_no,MaxFEs,lb,ub,dim,fobj)
...
while  FEs < MaxFEs
	...
	[X1,FEs]= HHO(Positions,MaxFEs,FEs,Alpha_pos,SearchAgents_no,fobj,ub,lb,dim);
    ...
    X_CMA = sorted_pos(1:size(Positions,1),:);
	[Positions,FEs] = Best_Position(Positions,X_CMA,X1,FEs,fobj);
    for i=1:size(Positions,1)
...
function [X,FEs] = Best_Position(X,X1,X2,FEs,fobj)
n=size(X,1);
for i=1:n
	FEs = FEs + 3;
    y_X=fobj(X(i,:));
...
function [X,FEs]= HHO(Positions,MaxFEs,FEs,Alpha_pos,N,fobj,ub,lb,dim)
 a=2-FEs*((2)/MaxFEs); % a decreases linearly fron 2 to 0
 for i=1:size(Positions,1)
    E0 = 2 * rand() - 1;
    E = a * E0;
	if abs(E) >= 1
		q = rand();
		r_index = floor(N * rand() + 1);
		Pos_temp = Positions(r_index,:);
		if q < 0.5
			%display(q);
			Positions(i,:)= Pos_temp - rand()*abs(Pos_temp - 2 * rand() * Positions(i,:));
		elseif q >= 0.5
			%display(q);
			Positions(i,:) = (Alpha_pos - mean(Positions) - rand()*(ub - lb)*rand() + lb);
		end
	elseif abs(E) < 1
		r = rand();
		if r >= 0.5 && abs(E) < 0.5
			% display(r);
			Positions(i,:) = Alpha_pos - E * abs(Alpha_pos - Positions(i,:));
		end
		if r >= 0.5 && abs(E) >= 0.5
			J = 2 * (1 - rand());
			%display(r);
			Positions(i,:) = (Alpha_pos -  Positions(i,:)) - E * abs(J * Alpha_pos - Positions(i,:));
		end
		if r < 0.5 && abs(E) >= 0.5
			J = 2 * (1 - rand());
			Pos1 = Alpha_pos - E * abs(J * Alpha_pos - Positions(i,:));
			% FEs = FEs + 1;
			if fobj(Pos1) < fobj(Positions(i,:))
			%display(r);
				Positions(i,:) = Pos1;
			else
				Pos2 = Alpha_pos - E * abs(J * Alpha_pos - mean(Positions)) + rand(1,dim).* L(dim);
				if fobj(Pos2) < fobj(Positions(i,:))
					%display(r);
					Positions(i,:) = Pos2;
				else
					Positions(i,:) = Alpha_pos;
				end
			 end
		end
		if r < 0.5 && abs(E) < 0.5
			J = 2 * (1 - rand());
			Pos1 = Alpha_pos - E * abs(J * Alpha_pos - mean(Positions));
			% FEs = FEs + 1;
			if fobj(Pos1) < fobj(Positions(i,:))
				 %display(r);
				Positions(i,:)= Pos1;
