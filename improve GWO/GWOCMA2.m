...
% beta混沌  a = 2 + (0 - 2)*((1-FEs/MaxFEs)^k1)^k2; 动态权重更新
% Grey Wolf Optimizer
function [Alpha_pos,Convergence_curve]=GWOCMA2(SearchAgents_no,MaxFEs,lb,ub,dim,fobj)
...
%         end    
        a = 2 + (0 - 2)*((1-FEs/MaxFEs)^k1)^k2;
        %a = 2 * sqrt(1-(FEs/MaxFEs)^2);
        %a = 2 - (0 - 2)*(FEs/MaxFEs)^2;
        %a = 2 - 2 * (1/(exp(1)-1)*exp(FEs/MaxFEs -1));
