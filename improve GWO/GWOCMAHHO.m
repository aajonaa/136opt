...
%在GWOCMA基础上的前期GWO加上HHO做贪心选择
% Grey Wolf Optimizer
function [Alpha_pos,Convergence_curve]=GWOCMAHHO(SearchAgents_no,MaxFEs,lb,ub,dim,fobj)
...
while  FEs < MaxFEs
    ...
    if FEs < MaxFEs/3               
       ...
       if abs(E) >= 1
           ...
           if q < 0.5
               %display(q);
               temp1 = Pos_temp - rand()*abs(Pos_temp - 2 * rand() * Positions(i,:));
           elseif q >= 0.5
               %display(q);
               temp1 = (Alpha_pos - mean(Positions) - rand()*(ub - lb)*rand() + lb);
           end
       elseif abs(E) < 1
           r = rand();
           if r >= 0.5 && abs(E) < 0.5
              % display(r);
               temp1 = Alpha_pos - E * abs(Alpha_pos - Positions(i,:));
           end
           if r >= 0.5 && abs(E) >= 0.5
               J = 2 * (1 - rand());
               %display(r);
               temp1 = (Alpha_pos -  Positions(i,:)) - E * abs(J * Alpha_pos - Positions(i,:));
           end
           if r < 0.5 && abs(E) >= 0.5
               J = 2 * (1 - rand());
