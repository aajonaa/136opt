function [best_pos,Convergence_curve]=PLOmod3(N,MaxFEs,lb,ub,dim,fobj)
% Add other generation + mechanism
% Told add other generation is ok
...
%             newX(i,:) = X(i, :) + (X(i,:)-Best_pos) .* cauchy_value(:,dim);%%3
%         else
...
%         end
    ...
    for i=1:N
        ...
        while true, r2 = 10 + round((N - 10) * rand + 0.5); if r2 ~= i && r2 ~= bestInd && r2 ~= r1, break, end, end
        for j=1:dim
%             if rand <= eta_qKR(i)
