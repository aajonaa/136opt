%% V1 version add the boxplot sheet files
%% Initial setting
...
% algorithmName = {'MSAO', 'AO0', 'CLPSO'} %#ok<NOPTS>
...
Flod=10;
NumofRecord = 40;
flodNum=size(algorithmName,2);
funcNum=size(Function_name_all,2);
bestFitnessAll = inf * ones(flodNum, Flod, funcNum);
...
dirName = ['WangJianFolder/expResult/', dayStr, ...
...
%     Function_Title_name=FunctionSeq_name; % Test using
    ...
    all_curves=zeros(flodNum * Flod, NumofRecord);
    for algo = 1:flodNum
    ...
    YValue = zeros(flodNum, NumofRecord);
    for algo = 1:flodNum
...
for algo = 1:flodNum-1 % The first row
...
for algo = 1:flodNum-1 % The data table
...
pValueSign = cell(funcNum+2, flodNum);
pValueSign{1, 1} = 'Func';
...
for algo = 1:flodNum-1 % The pValue and sign table
    cntBetter = 0;
    cntWorse = 0;
    for func = 1:funcNum
...
%% To write the flod final fitness for boxplot
for func = 1:funcNum
    data = cell(Flod, flodNum);
    data{1, 1} = 'Flod';
    for algo = 1:flodNum
    ...
    for algo = 1:flodNum
        for flod = 1:Flod
            data{flod+1, 1} = ['flod', num2str(flod)];
            data{flod+1, algo+1} = bestFitnessAll(algo, flod, func);
        end
    end
