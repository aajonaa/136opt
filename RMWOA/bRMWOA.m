...
function [bestFitness, best_pos, Convergence_curve, Time] = bRMWOA(N, Max_FEs, dim, AData, trn, vald, TFid, classifierFhd)
    ...
    %     AllFitness(i, 1)=fobj(X(i,:));
        AllFitness(i, 1) = AccSz2(X(i,:), AData, trn, vald, classifierFhd);
    %     FEs=FEs+1;
    ...
    while  t < Max_FEs
        ...
        a=2-t*((2)/Max_FEs); % a decreases linearly fron 2 to 0 in Eq. (2.3)
    ...
    %                     X(i, j) = X(i, j) - A * abs(C * X(floor(N*rand()+1), j) - X(i, j));
                        temp = X(i, j) - AData * abs(C * X(floor(N*rand()+1), j) - X(i, j));
                        X(i, j) = transferFun(X(i, j), temp, TFid);

                    elseif abs(AData)<1 % exploitation
    ...
    %                     X(i,j) = X(Rolette_index, j) + A * abs(C * X(Rolette_index, j) - X(i,j));      % Eq. (2.2)
                        temp = X(Rolette_index, j) + AData * abs(C * X(Rolette_index, j) - X(i,j));      % Eq. (2.2)
                        X(i, j) = transferFun(X(i, j), temp, TFid);
                    end
    ...
    %                 X(i,j)= abs(best_pos(j) - X(i,j)) * exp(b*l) * cos(l*2*pi) + best_pos(j); 
                    temp = abs(best_pos(j) - X(i,j)) * exp(b*l) * cos(l*2*pi) + best_pos(j); 
                    X(i, j) = transferFun(X(i, j), temp, TFid);
    ...
    %         X2(i, :) = history_X(i, :) + randn * m .* (history_X(Rolette_index, :)- history_X(i, :));
            ...
            X2(i, :) = transferFun(X2(i, j), temp, TFid);
        end
    ...
    %         fitness = fobj(X2(i, :));
            fitness = AccSz2(X2(i, :), AData, trn, vald, classifierFhd);
