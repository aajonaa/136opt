function [Best_P, Convergence_curve] = G111(N, MaxFEs, lb, ub, dim, fobj)
...
update_interval = 6;           % Frequency of graph structure updates
...
while FEs <= MaxFEs
    ...
    message_passing_strength = 0.5 * 2^(1 - progress_ratio);
    ...
    for i = 1:N
        ...
        if ~isempty(neighbors)
            ...
            Node1 = P(i,:) + message_passing_strength * R .* (w1 * (Best_P - LocalMean) + (1-w1) * (LocalMean - P(i,:)));
            
            % Rule 2: Global influence
            Node2 = Best_P + message_passing_strength * R .* (w1 * (LocalMean - P(i,:)) + (1-w1) * (P(i,:) - Best_P));
            
            % Rule 3: Local influence
            Node3 = LocalMean + message_passing_strength * R .* (w1 * (P(i,:) - Best_P) + (1-w1) * (Best_P - LocalMean));
