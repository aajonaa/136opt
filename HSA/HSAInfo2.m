...
function [Best_P, Convergence_curve] = HSAModule(N,MaxFEs,lb,ub,dim,fobj)
%% 初始化参数
Best_P = zeros(1,dim);
Best_fit = inf;
...
while FEs <= MaxFEs
    ...
    for i = 1:N
        for j = i+1:N
            ...
            similarity = CalculateSimilarity(similarity, P, i, j, FEs, MaxFEs);
...
%     similarity(i, i) = 0.1;
% end
% function similarity = CalculateSimilarity(similarity, P, i, j)
%     % Mutual Information estimation
%     jointProb = hist3([P(i,:); P(j,:)]', [10, 10]);
%     jointProb = jointProb / sum(jointProb(:));
% 
%     % Entropy calculation
%     entropy = -sum(jointProb(:) .* log(jointProb(:) + eps));
% 
%     % Normalized similarity
%     similarity(i, j) = 1 / (1 + entropy);
% end
function similarity = CalculateSimilarity(similarity, P, i, j, FEs, MaxFEs)
    % Calculate combined similarity between two solutions using mutual information
    % and Euclidean distance
    %
    % Inputs:
    %   P - Solution matrix
    %   i - Index of first solution
    %   j - Index of second solution
    %
    % Output:
    %   combinedSimilarity - Combined similarity score between 0 and 1

    % Get the two solution vectors
    vec1 = P(i,:);
    vec2 = P(j,:);

    % Calculate information-theoretic similarity
    dim = size(P, 2);
    
    % Initialize discrete vectors
    discreteVec1 = zeros(1, dim);
    discreteVec2 = zeros(1, dim);

    % Discretize each dimension
    for d = 1:dim
        % Use fixed-interval discretization to divide into 4 intervals from -100 to 100
        intervalWidth = (100 - (-100)) / 4;
        edges = -100:intervalWidth:100;
        edges(end) = []; % Remove last value for histc function
        
        % Discretize values using histc
        [~, discreteVec1(d)] = histc(vec1(d), edges);
        [~, discreteVec2(d)] = histc(vec2(d), edges);
    end

    % Handle cases where values fall outside the bins
    discreteVec1(discreteVec1 == 0) = 1;
    discreteVec2(discreteVec2 == 0) = 1;

    % Compute joint histogram
    jointCount = accumarray([discreteVec1; discreteVec2]', 1);

    % Normalize to get probability
    jointProb = jointCount ./ sum(jointCount(:));

    % Compute entropy
    % Avoid log(0) by adding small epsilon
    entropy = -sum(jointProb(:) .* log(jointProb(:) + eps));

    % Convert entropy to similarity
    % Lower entropy means more similar
    infoSimilarity = 1 / (1 + entropy);

    % Ensure similarity is between 0 and 1
    infoSimilarity = max(0, min(1, infoSimilarity));

    % Calculate Euclidean distance
    euclideanDistance = norm(vec1 - vec2);
    
    % Normalize Euclidean distance to [0,1] range
    % Maximum possible distance is sqrt(30 * (200)^2) when all variables are at opposite extremes
    maxPossibleDistance = sqrt(dim * 200^2);
    normalizedDistance = euclideanDistance / maxPossibleDistance;
    distanceSimilarity = 1 - normalizedDistance;

    % Combine information similarity and Euclidean distance
    % Using weighted average (weights can be adjusted based on requirements)
    weightForInfo = FEs/MaxFEs;  % Weight for information similarity
    weightForDistance = 1 - FEs/MaxFEs;  % Weight for Euclidean distance similarity

    % Calculate combined similarity
    combinedSimilarity = weightForInfo * infoSimilarity + ...
                        weightForDistance * distanceSimilarity;

    % Ensure final similarity is between 0 and 1
    similarity(i, j) = max(0, min(1, combinedSimilarity));
end
