...
while FEs <= MaxFEs
    ...
    for i = 1:N
        for j = i+1:N
            ...
            similarity = calculateSimilarity(similarity, P, i, j, lb, ub, FEs, MaxFEs);
...
%     similarity(i, i) = 1;
% end
function similarity = calculateSimilarity(similarity, P, i, j, lb, ub, FEs, MaxFEs)
    % Euclidean distance (normalized)
    normDist = norm((P(i,:) - P(j,:)) ./ (ub - lb)) / sqrt(size(P,2));
    
    % Cosine similarity
    dotProduct = dot(P(i,:), P(j,:));
    normI = norm(P(i,:));
    normJ = norm(P(j,:));
    cosineSimilarity = dotProduct / (normI * normJ);
    
    % Combined metric
    similarity(i, j) = 1 - (FEs/MaxFEs * normDist + (1-FEs/MaxFEs) * (1 - cosineSimilarity));
end
