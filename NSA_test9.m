...
function [best_pos,Convergence_curve] = NSA(N, Max_FEs, lb, ub, dim, fobj)
    ...
    while FEs < Max_FEs
        ...
        %% Upper social: localElite不同维交换，交换位置不固定，交换数量为1 + localElite多维交换，交换位置固定，交换数量不固定
        social_X = current_X;
            ...
            for i = 1:N
                if flag == 0
                     for j = 1:dim
                        if m(j)
                            social_X1 = localElite_X(i, j);
                            social_X2 = best_pos(j);
                            social_X(i, j) = (social_X1 + social_X2) / 2;
%                             social_X(i, j) = localElite_X(i, j);
        ...
        for i = 1:N
            ...
            if social_Fitness(i, 1) < current_Fitness(i, 1)
                % Social success: apply multi-dimension source exchange
                flag(i, 1) = 1;
                current_X(i, :) = social_X(i, :);
            else
                % Social fail: apply one-dimension source exchange
                flag(i, 1) = 0;
            end
        end
