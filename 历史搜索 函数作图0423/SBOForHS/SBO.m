...
% 2024-3-18.
% Version for history search.
function  [bestFitness, best_pos,Convergence_curve, Trajectories,fitness_history, position_history] = SBO(N,Max_FEs,lb,ub,dim,fobj)
...
%         FEs = FEs + 1;
    ...
    position_history=zeros(N,Max_FEs,dim);
    Trajectories=zeros(N,Max_FEs);
    %%
...
%     while FEs < Max_FEs
        ...
        for i = 1:N
            r1 = randn;
            r2 = randn;
%             r3 = tanh(sqrt(abs(Max_FEs - randn * iter))/N);
%             r3 = tanh(sqrt(Max_FEs - randn * iter)/N);
            r3 = tanh((Max_FEs - iter)/(iter/i));
            r4 = unifrnd(-r3, r3);
            if rand < r3
                for j = 1:dim
...
%             FEs = FEs + 1;
        ...
        %%
        for i = 1:N
            fitness_history(i,iter)=current_Fitness(i);
            position_history(i,iter,:)=current_X(i,:);
            Trajectories(:,iter)=current_X(:,1);
        end
        %%
