...
function [best_pos,convergence_curve]=HHSMA(N,Max_FEs,lb,ub,dim,fobj)
tic
disp('HHSMA_origin is now tackling your problem')
...
%             X(i,:) = (ub-lb)*rand+lb;
        ...
        %%%%%%%%%%
        % Apply the HHO's exploration method to the SMA.
        if rand<z && abs(Escaping_Energy) >= 1 
%             X(i,:) = (ub-lb)*rand+lb;
            q = rand();
...
%             elseif q>=0.5   % perch on a random tall tree (random site inside group's home range)
                ...
                X(i,:)=(best_pos-mean(X))-rand()*((ub-lb)*rand+lb);
            end
        %%%%%%%%%%
