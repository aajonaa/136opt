...
function [best_pos,convergence_curve]=SMA_l2(N,Max_FEs,lb,ub,dim,fobj)
tic
disp('SMA_l2 is now tackling your problem')
...
for i = 1:N
    if i == 1
        ...
        mu = 4 * rand();
        for j = 1:dim
            X(i, j) = 4 / mu * (X(i-1, j) - lb) / (ub - lb) * (1 - (X(i-1, j) - lb) / (ub - lb));
        end
    end
end
...
%         gamma = 0.5 * rand();
%         for j = 1:dim
%             X(i, j) = gamma * min((X(i - 1, j) - lb) / (ub - lb), 1 - (X(i-1, j) - lb) / (ub - lb));
%         end
%     end
% end
