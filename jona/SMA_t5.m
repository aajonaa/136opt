...
% 1. gamma = 2, + rand() * 1 / N；
function [best_pos,convergence_curve]=SMA_t5(N,Max_FEs,lb,ub,dim,fobj)
tic
disp('SMA_t5 is now tackling your problem')
...
%         gamma = 0.5 * rand();
        for j = 1:dim
            X(i, j) = 2 * min((X(i - 1, j) - lb) / (ub - lb), 1 - (X(i-1, j) - lb) / (ub - lb)) + rand() * 1 / N;
        end
    end
end
