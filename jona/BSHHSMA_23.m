% Slime Mold Algorithm modified by Jona 2023-11-6.
...
% HHO. 13.Change the random point A to the average X. 14.Cancel 13 and when
% rand < z, I changed the mechanism.
function [best_pos,convergence_curve]=BSHHSMA_23(N,Max_FEs,lb,ub,dim,fobj)
tic
disp('BSHHSMA_23 is now tackling your problem')
...
% Bad result. Convergence to the premature optimal.
%                     X(i,j) = best_pos(j)+ vb(j)*(weight(i,j)*X_avg(j)-X(B,j));
