...
% Destination fitness in order to draw it in convergence_curve. Update the
% best_pos after update the agent.
function [best_pos,convergence_curve]=EOBLHHSMA_6_2(N,Max_FEs,lb,ub,dim,fobj)
tic
disp('EOBLHHSMA_6_2 is now tackling your problem')
...
while  FEs < Max_FEs
    ...
    for i=1:N
        ...
        %             elseif q>=0.5   % perch on a random tall tree (random site inside group's home range)
            ...
            FEs = FEs + 1;
            fitness_Xi = fobj(X(i, :));
            if fitness_Xi < Destination_fitness
                best_pos = X(i, :);
            end
        end
    end
...
%     %%%%%%%%%%
    ...
    fitness_Temp = fobj(Temp);
    FEs = FEs + 1;
    if fitness_Temp < Destination_fitness
        Destination_fitness = fitness_Temp;
        best_pos = Temp;
    end
