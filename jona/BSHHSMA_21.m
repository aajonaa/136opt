% Slime Mold Algorithm modified by Jona 2023-11-3.
...
% HHO. 13.Make some change taking the first improved algorithm as
% reference and the two weight slime mode algorithm.
function [best_pos,convergence_curve]=BSHHSMA_21(N,Max_FEs,lb,ub,dim,fobj)
tic
disp('BSHHSMA_21 is now tackling your problem')
...
%     end
    ...
    for i=1:size(X,1)
        for j=2:size(X,2)
            if i<=(size(X,1)/2)
                weight(sorted_fitness_index(i),j) = 1+rand()*log10((bestFitness-sorted_fitness_value(i))/(bestFitness-worstFitness)+1);
            else
                weight(sorted_fitness_index(i),j) = 1-rand()*log10((bestFitness-sorted_fitness_value(i))/(bestFitness-worstFitness)+1);
            end
            if sorted_fitness_value(i)>mean(AllFitness)
                weight1(i,:) = 1+(worstFitness-sorted_fitness_value(i))/(worstFitness-bestFitness);
            else
                weight1(i,:) = 1.000001-(worstFitness-sorted_fitness_value(i))/(worstFitness-bestFitness);
            end
        end
    end
