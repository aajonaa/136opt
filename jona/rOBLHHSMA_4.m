...
% Destination fitness in order to draw it to the convergence_curve.
function [best_pos,convergence_curve]=rOBLHHSMA_4(N,Max_FEs,lb,ub,dim,fobj)
tic
disp('rOBLHHSMA_4 is now tackling your problem')
...
while  FEs < Max_FEs
    ...
    %%%%%%%%%%
    for i = 1:N  
        new_X(i, :) = rand() * (ub + lb) - X(i, :);
        for j = 1:dim
           if new_X(i,j) < lb(j) || new_X(j) > ub(j)
    ...
    [sorted_fitness_value, index] = sort(fitness_value);
    X = population2(index, :); 
    X = X(1:N, :); 
    
    % Update the Destination fitness.
...
%     population2 = [X; population1];
%     for i = 1:2*N
%         fit_value(i) = fobj(population2(i, :));
%     end
%     [~, index] = sort(fit_value);
%     X = population2(index, :); 
%     X = X(1:N, :); 
%     %%%%%%%%%%
%     
