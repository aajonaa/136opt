...
% Modified by Jona at 2023-11-12, I use the mutation and crossover at the
...
function [best_pos,convergence_curve]=BSSMA_27(N,Max_FEs,lb,ub,dim,fobj)
tic
disp('BSSMA_27 is now tackling your problem')
...
while  FEs < Max_FEs
    ...
    for i=1:N
        ...
        Pch = X(i, :) + (map * F) * (X_mean - X);
        if fobj(Pch) < fobj(X(i, :))
            FEs = FEs + 2;
            X(i, :) = Pch;
        end
        %%%%%%%%%%
