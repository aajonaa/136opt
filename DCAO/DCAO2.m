...
% =============================================================================================================================================================================
function [best_pos,convergence_curve] = DCAO2(N,MaxFEs,Lb,Ub,dim,fobj)
    Lb = Lb * ones(1, dim);
    Ub = Ub * ones(1, dim);
    % Parameters
    next_X = zeros(N,dim);
            ...
            elseif i <= ngS %%%%%
                ...
                for d = 1:dim
                    if rand <= eta_qKR(i) || d == jrand
                        next_X(i,d) = X(r1,d) + LnF3(golden_ratio,0.05,1,1);
                    end
                end
            else
                % Average-performing
                ...
                for d = 1:dim
                    if rand <= eta_qKR(i) || d == jrand
                        next_X(i,d) = X(bestInd,d) + ((X(r2,d) - X(i,d)) * lamda_t) + ((X(r1,d) - X(i,d)) * omega_it);
                    end
                end
            end
            % Boundary
            next_X(i,:) = boundConstraint(next_X(i,:),X(i,:),[Lb; Ub]);
            new_X(i, :) = next_X(i, :);
            new_fitness(i,1) = fobj(next_X(i, :));
            FEs = FEs + 1;
