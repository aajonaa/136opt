...
function [Leader_pos,Convergence_curve]=AO(N,MaxFEs,lb,ub,dim,fobj)
...
function  z = mutation2(z, N, dim, input_pop, input_fitness, FEs, MaxFEs)
    ...
    for d = 1:dim
        if rand <= eta_qKR(i)
            z(d) = X(bestInd,d) + ((X(r2,d) - X(i,d)) * lamda_t) + ((X(r1,d) - X(i,d)) * omega_it); %%% 3
            % z = X(bestInd,d) + ((X(r2,d) - X(i,d)) * lamda_t) + ((X(r1,d) - X(i,d)) * omega_it); %%% 3
        end
    end
end