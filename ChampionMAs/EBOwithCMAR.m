...
%%==============================================================================
function [bestx,Convergence_curve]= EBOwithCMAR2(N,Max_FEs,lb,ub,dim,fobj)
% function [bestx,Convergence_curve]= EBOwithCMAR(N,Max_iteration,lb,ub,dim,fhd,funcNum)


Par= Introd_Par(N,lb,ub,dim,Max_FEs);
...
tic;
% fitx = cec17_func(x',funcNum);
% fitx= feval(fhd, x', funcNum);
for i=1:size(x,1)
    fitx(i)=fobj(x(i,:));
    FEs=FEs+1;
end
...
%     Probs = [1 1];
    ...
    if (current_eval<Par.Max_FES)
        if rand<Probs(1)
            ...
            %% apply EBO
            [EA_1, EA_1old, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, current_eval,res_det,FEs] = ...
                EBO( EA_1,EA_1old, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,....
                archive_freq, Par.xmin, Par.xmax,  Par.n,  PS1,  current_eval,res_det,Par.Printing,Par.Max_FES, Par.Gmax, FEs,fobj,iter);
        end
    end
    ...
    if (current_eval<Par.Max_FES)
        if   rand<Probs(2)
            [ EA_2, EA_obj2, setting,bestold,bestx,bnd,fitness,current_eval,res_det,FEs] = ...
                Scout( EA_2, EA_obj2, probSC, setting, iter,bestold,bestx,fitness,bnd,...
                Par.xmin,Par.xmax,Par.n,PS2,current_eval,res_det,Par.Printing,Par.Max_FES,fobj,FEs);
%             disp([min(EA_obj2) Probs(1) Probs(2)]);
    ...
    if current_eval>0.75*Par.Max_FES
        if rand<Par.prob_ls
            ...
            [bestx,bestold,current_eval,succ] = LS2 (bestx,bestold,Par,current_eval,Par.Max_FES,Par.xmin,Par.xmax,fobj);
...
%             fprintf('in problem: %d, there is  a violation',funcNum);
%         end
%         SR= (outcome==0);
%     end
...
% Some part of this code is taken from UMOEA-II
% =========================================================================
function [Par] = Introd_Par(N1,lb_bound,ub_bound,DIM,Max_FEs)
...
Par.xmin= lb_bound*ones(1,Par.n);
Par.xmax= ub_bound*ones(1,Par.n);
Par.Max_FES=Par.n*200000;
% Par.f_optimal=opt(fnum);
% Par.PopSize=18*Par.n; %% population size
Par.PopSize=N1;%% population size
Par.MinPopSize=4;
Par.prob_ls=0.1;
...
function[ x, fitx, setting,bestold,bestx,bnd,fitness,current_eval,res_det,FEs] = ...
    Scout( x, ~, prob, setting, iter,bestold,bestx,fitness,bnd,xmin,xmax,n,PopSize,current_eval,res_det,Printing,Max_FES,fobj,FEs)
...
% fitness.raw= feval(fhd, arxvalid, I_fno);
for i=1:size(arxvalid,2)
    fitness.raw(i)=fobj((arxvalid(:,i))');
    FEs=FEs+1;
end
...
if  (setting.ccov1+setting.ccovmu+neg.ccov) > 0 && mod(iter, 1/(setting.ccov1+setting.ccovmu+neg.ccov)/n/10) < 1
    ...
    data11=isnan(setting.C);
    if any(data11(data11==1))
        setting.C = setting.Cn;
    end
    setting.Cn = setting.C;
...
% =========================================================================
function [x,f,current_eval,succ] = LS2 (bestx,f,Par,current_eval,Max_FES,xmin,xmax,fobj)
...
[Xsqp, FUN , ~ , details]=fmincon(fobj, bestx(1,:),[],[],[],[],xmin,xmax, [],options);
...
function [x, xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq,current_eval,res_det,FEs ] = ...
    EBO( x,xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, xmin, xmax,  n,...
    PopSize,  current_eval,res_det,Printing,Max_FES, G_Max, FEs ,fobj,gg)
