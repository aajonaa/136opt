...
Convergence_curve = [];
avg_fitness_curve = zeros(1, Max_FEs);
GBestX = X(1, :); % Global best position
X_new = X;
search_history = zeros(N, Max_FEs, dim);
fitness_history = zeros(N, Max_FEs);
%% Start search
while FEs < Max_FEs
%     if mod(i,100) == 0
...
%     end
    ...
    for j = 1:size(X, 1)
        ...
        % foraging behavior
        if St == 1
                X_new(j, :) = (X(j, :) - GBestX) .* Levy(dim) + rand(1) * mean(X) * (1 - i / Max_FEs) ^ (2 * i / Max_FEs);

        % staying behavior
        elseif St == 2
                X_new(j, :) = X(j, :) + GBestX .* Levy(dim) + randn() * (1 - i / Max_FEs) * ones(1, dim);
        ...
        % fear of strangers' behavior
                ...
                X_new(j, :) = X(j, :) + rand() * cos((pi *i )/ (2 * Max_FEs)) * (GBestX - X(j, :)) - cos(sita) * (i / Max_FEs) ^ (2 / Max_FEs) * (X(j, :) - GBestX);
        end
...
%         X = randi([0, 1], 30, 30);  % 生成一个30x30的随机矩阵，元素值为0或1
        uniqueRows = unique(X, 'rows');  % 找出唯一的行向量
        numUniqueRows = size(uniqueRows, 1);  % 获取唯一行向量的数量
        disp(['Number of unique rows: ', num2str(numUniqueRows)]);  % 显示唯一行向量的数量
