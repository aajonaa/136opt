function [best_pos,Convergence_curve]=RIME(N,Max_FEs,lb,ub,dim,fobj)
    ...
    while FEs < Max_FEs
        ...
        [sorted_Fitness, ~] = sort(AllFitness);
        Roulette_index=RouletteWheelSelection(1./(sorted_Fitness + eps));
        if Roulette_index==-1  
            Roulette_index=1;
        end
        ...
        F = randn;%Parameters of Eq.(3),(4),(5)
        ...
        %     w2 = randn;
        %     for j = 1:dim
        %         newX2(i,j) = (1 - w1 - w2) * X(i, j) + w1 * X(Roulette_index, j) + w2 * best_pos(j);
        %     end
        % end
        % newX2 = newX;
        % 
        ...
        %     newX2(i,:)=(newX2(i,:).*(~(Flag4ub+Flag4lb)))+ub.*Flag4ub+lb.*Flag4lb;
        %     newAllFitness(1,i)=fobj(newX(i,:));
        %     newAllFitness2(1,i)=fobj(newX2(i,:));
        %     FEs=FEs+2;
        %     %Positive greedy selection mechanism
        %     if newAllFitness(1,i)<newAllFitness2(1,i)
        %         newAllFitness3(1,i) = newAllFitness(1,i);
        %     else
        %         newAllFitness3(1,i) = newAllFitness2(1,i);
        %         newX(i, :) = newX2(i, :);
        %     end
        % end
