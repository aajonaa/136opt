...
%
% DMS_PSO：
% 论文引自：Dynamic multi-swarm particle swarm optimizer with local search for large scale global optimization
function [gbest, cg_curve]= DMS_PSO(N,Max_iteration,lb,ub,dim,fobj)
...
i=0;
cnt = 1;
% while fitcount<0.95*Max_FES && cnt <= Max_iteration
...
%     end
    ...
    if mod(i,100)==0
        ...
        for k=1:L_num
            [x,fval,exitflag,output] = fminunc(fobj,gbest(tmpid(k),:),options);
...
aaa=[];bbb=[];ggg=[];
    ...
    [tmp,tmpid]=sort(gbestval);
    gbest_val=gbestval(tmpid(1));
    cg_curve(cnt)=gbest_val;
    cnt = cnt + 1;
end
[tmp,tmpid]=sort(gbestval);
gbest=gbest(tmpid(1),:);
...
[x,fval,exitflag,output] = fminunc(fobj,gbest,options);
fitcount=fitcount+output.funcCount;
if fval<gbestval
    gbest=x;
    gbestval=fval;
...
% eval(['save gbest_record_' num2str(func_num) '_' num2str(run) ' gbest_record']);