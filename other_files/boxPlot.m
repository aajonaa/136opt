...
% filename = 'AFeature.xlsx';  
funcNum = 3;
for func = 1:funcNum
    h = figure(func);
    % D4 = xlsread(filename,i);
    D4 = readmatrix('MSAO.xlsx', 'sheet', ['F', num2str(func)]);
    D4 = D4(:, 2:end);
    aboxplot(tickLabelStr{func},D4,'labels',{'BCPVSMA','BSCA','BBA','BSSA','BSMA','bMFO'});
    ylim('auto')
    fontSize = 10;
    %x-labels
...
function aboxplot(titlename,X,varargin)
    widthl = 0.7;
    widths = 0.8;
    widthe = 0.4;
    outMarker = '.';
    ...
    set(allaxes,'FontName','Times','FontSize',12);
    set(alltext,'FontName','Times','FontSize',12);
    ...
    print('-depsc2', '-r300','E'); %<-Save as PNG with 300 DPI
