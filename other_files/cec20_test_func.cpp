...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    // Validate inputs
    if (nrhs != 2)
        mexErrMsgTxt("Requires 2 inputs: function number and x vector.");
    
    // Get function number from first input
    int func_num = static_cast<int>(mxGetScalar(prhs[0]));
    
    // Get x vector from second input
    double* x = mxGetPr(prhs[1]);
    int dim = static_cast<int>(mxGetNumberOfElements(prhs[1]));
    
    // Validate function number
    if (func_num < 1 || func_num > 10)
        mexErrMsgTxt("Function number must be between 1 and 10.");
    
    // Prepare output (fitness value)
    plhs[0] = mxCreateDoubleMatrix(1, 1, mxREAL);
    double* f = mxGetPr(plhs[0]);
    
    // Call your CEC20 test function
    cec20_test_func(x, f, dim, 1, func_num);  // Adjust this call to match your actual function
}
