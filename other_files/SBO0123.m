...
function [best_pos,Convergence_curve] = SBO(N, Max_FEs, lb, ub, dim, fobj)
    ...
    while FEs < Max_FEs
        ...
        for i = 1:N
            r1 = randn;
            r2 = randn;
            r3 = tanh(sqrt(abs(Max_FEs - FEs))/N);
            r4 = unifrnd(-r3, r3);
            if rand < r3
                % for j = 1:dim
                    % current_X(i, j) = (1 - r1 - r2) * current_X(i, j) + r1 * localElite_X(Rolette_index, j) + r2 * best_pos(j);
                    current_X(i, :) = (1 - r1 - r2) * current_X(i, :) + r1 * localElite_X(Rolette_index, :) + r2 * best_pos;
                % end
            else
                % for j = 1:dim
                    % current_X(i, j) = r4 * ((1 - r1 - r2) * current_X(i, j) + r1 * localElite_X(Rolette_index, j) + r2 * best_pos(j));
                    current_X(i, :) = r4 * ((1 - r1 - r2) * current_X(i, :) + r1 * localElite_X(Rolette_index, :) + r2 * best_pos);
                % end
            end
        end
