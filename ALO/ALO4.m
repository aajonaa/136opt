...
%     This for loop simulate random walks
    for i=1:size(ant_position,1)
        ...
        RA=Random_walk_around_antlion4(dim,Max_FEs/dim,lb,ub, Sorted_antlions(Rolette_index,:),iter);
        
        % RE is the random walk around the elite (best antlion so far)
        RE=Random_walk_around_antlion4(dim,Max_FEs/dim,lb,ub, Elite_antlion_position(1,:),iter);
