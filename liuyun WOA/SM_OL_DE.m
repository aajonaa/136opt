...
% -------------------------------------------------------------------------- %
function [Leader_pos,Convergence_curve]=SM_OL_DE(SearchAgents_no,Max_iter,lb,ub,dim,fobj)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%引入差分算法的交叉变异，选择机制%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i=1:reNum
        ...
        [OLPositions(i,:),Leader_pos,Leader_score,Secondary_pos,Secondary_score,Loser_score,FEs] = DEfun(Positions(i,:),V,Leader_pos,Leader_score,Secondary_pos,Secondary_score,Loser_score,f,dim,FEs,fobj);
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    ...
    for i=1:size(Positions,1)
        ...
        %%%%%%%%%%%%%%%%%%%利用单纯形法更新位置向量%%%%%%%%%%%%%%%%%%%%%%
        [SimplexPosition,Leader_score,Leader_pos,FEs] = Simplex(Leader_pos,Leader_score,Secondary_pos,Positions(i,:),FEs,fobj);
...
%%%%%%%%%%%%%%%%%%%单纯形法%%%%%%%%%%%%%%%%%%%%
function [Loser_pos,Leader_score,Leader_pos,FEs] = Simplex(Leader_pos,Leader_score,Secondary_pos,Loser_pos,FEs,fobj)
    Loser_score = fobj(Loser_pos);
    FEs = FEs + 1;
    if Loser_score < Leader_score
        Leader_score = Loser_score;
        Leader_pos = Loser_pos;
    end
    alpha = 1;
    gamma = 2;
    beta = 0.5;
