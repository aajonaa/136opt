function [PSMADE_position,PSMADE_curve]=PSMADE(n,Max_iteration,lb,ub,dim,fobj)
...
while  FEs < Max_iteration%%
    ...
    %% DE mechanism
    [FEs, x,pb,all_fitness,pb_fit] = funcDE(FEs, fobj,x,pb,all_fitness,pb_fit,lb,ub,beta_min,beta_max);
    %%
...
function [FEs, x,pb,all_fitness,pb_fit] = funcDE(FEs, fobj,x,pb,all_fitness,pb_fit,lb,ub,beta_min,beta_max)
     %*****  Muation-Crossover  ****%
     n = size(x,1);
     dim=size(x,2);
