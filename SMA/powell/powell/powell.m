 ...
 while it < mxit,
  ...
  for i=1:n,
     if method,           % to see the linesearch plot, remove the two 0* below
       [stepsize,x,Ot,nS1]=aurea(S,xo,D(:,i),0*ips,problem,tol,mxit,none);
       Ot=Ot*problem;
     else
       [stepsize,x,Ot,nS1]=coggins(S,xo,D(:,i),0*ips,problem,tol,mxit,none);
       Ot=Ot*problem;
     end
  ...
  if di >= 0 | 2*(y0-2*yo+Ot)*((y0-yo-delta)/di)^2 >= delta,
    ...
    D(:,n)=(x-x0)/norm(x-x0);
    if method,           % to see the linesearch plot, remove the two 0* below
      [stepsize,xo,yo,nS1]=aurea(S,x,D(:,n),0*ips,problem,tol,mxit,none);
      yo=yo*problem;
    else
      [stepsize,xo,yo,nS1]=coggins(S,x,D(:,n),0*ips,problem,tol,mxit,none);
      yo=yo*problem;
    end
