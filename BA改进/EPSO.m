...
%
% EPSO：
% 论文引自：Ensemble particle swarm optimizer
function [gbest_pos,cg_curve]=EPSO(SearchAgents_no,MaxFEs,lb,ub,dim,fobj)
...
max_FES=100000;
max_iteration=MaxFEs;
num_particle=SearchAgents_no;
dimension=dim;
cg_curve=[];
FES = 0;
range=[lb,ub];
...
if fitcount<=(0.9*max_FES)
      ...
      for i=1:num_g1 
            ...
            if (sum(pos(i,:)>range_max(i,:))+sum(pos(i,:)<range_min(i,:))==0)
                result(i)=fobj(pos(i,:));% Evaluate fitness   
                 FES = FES + 1;
                fitcount=fitcount+1;
%                  result_data=result(i);
%                 result_data(fitcount)=result(i);
